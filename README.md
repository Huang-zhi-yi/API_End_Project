...
## 一.需求概述
 ...
 ## 三. 用户分析
   ...
   #### 产品原型介绍1-启动页
   点击页面上任意一个位置即可跳转到其他页面
   ### (2)核心页面
   ![聊天页面](./臻选美食界面/7_聊天页面.png)
   #### 产品原型介绍2-聊天页面
   用户进入app后会首先进入与机器人聊天页面(调用的是图灵机器人api),在聊天过程中用户可以点击发送文字也可以发送语音或者表情包，
   后台会根据用户发过来的文字使用百度对话情绪api识别用户的情绪，然后会根据心情为用户推荐合适的餐厅。
   ![聊天页面-输入界面](./臻选美食界面/3_聊天页面-输入框.png)
   #### 产品原型介绍3-聊天页面-输入框
   用户点击输入框后，界面会增加一个输入法区域，让用户可以输入文字
   ![主页](./臻选美食界面/6_主页.png)
   #### 产品原型介绍4-主页详细介绍
   * 用户跳转到主页后，顶部左边会有一个下拉选择框，用户可以切换城市或者国家，如果用户想寻找其他地方的餐厅，用户可以利用中间搜索框
   输入文字进行搜索。
   * 中间会有10个美食分类图标，如果用户对推荐餐厅类别不满意，可以点击图标，然后后台会用户重新进行推荐。中间的温馨提示
   则是会根据用户当前的心情，时间，为每位用户个性化提供一些温暖的话语，让用户感受到人文关怀。
   * 底部的滑动卡片即根据用户心情、用户饮食习惯推算出用户合适的餐厅类别，根据这个餐厅类别(typecode)结合定位调用高德地图周边搜索poi
   为用户推荐出最合乎他当前心情最合乎他口味的餐厅。用户可以左右滑动查看相关餐厅推荐，推荐中包含餐厅一些美食轮播图，餐厅地址、价格、距离等
   一般用户最为关心的信息，帮助用户第一时间了解到餐厅的基本情况。其中用户点击餐厅位置即可跳转到地图页面，用户可以及时了解到餐厅具体位置。
   ### (3)主要页面
   ![导航界面](./臻选美食界面/4_导航页.png)
   #### 产品原型介绍5-导航页
   用户可以查看到餐厅的具体位置，点击右下方即可打开各种地图软件，开启导航功能。
   ![我的](https://www.lucaszy.com/wp-content/uploads/2020/07/5_我的.png)
   #### 产品原型介绍6-个人主页原型介绍
   用户第一次进入app时是未登陆的，点击空头像即可进行登录操作。下方会有我的钱包、我的收藏、我的浏览等等按钮，登录后的用户点击这些按钮，
   可以看到自己的钱包余额，收藏记录、浏览记录等等，方便用户及时回忆起去过或者想去的餐厅和考虑是否要进行充值操作。用户可以点击我的反馈
